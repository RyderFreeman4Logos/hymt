...
When llama.cpp `/props` reports a one-character ASCII `eos_token` (for example, a malformed GGUF that reports `$`), hymt treats it as false EOS metadata. It resolves that character through `/tokenize` during preflight and sends llama.cpp-only `logit_bias` `{ "<token_id>": -100 }`, preventing the false stop without suppressing the actual chat-template EOS token (such as `<|eos|>`).
