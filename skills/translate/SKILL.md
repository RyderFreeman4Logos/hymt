...
- Each segment is validated for minimum output/input character ratio, paragraph retention, and Markdown heading preservation. CLI help/usage text gets a default-prompt hint that the help output is complete source text; dense translations can pass completeness when they preserve Usage/Options structure and enough `--long-option` anchors, while generic refusal text and examples-only truncation still fail. Failed segments are retried up to `[completeness].max_retries` (default `2`), the shared completeness retry setting for normal, streaming, batch, and `translate-doc`. After retries, a non-empty best attempt that meets the 33% approximate source-token floor is written as degraded best effort and emits `completeness_degraded_segments=…` on stderr; top-level text/file/stdin translation exits non-zero so scripts detect it. An empty or below-floor best attempt is unrecoverably incomplete and is rejected with an error instead of being written. Pass `--warn-only-completeness` or set `[completeness].warn_only = true` to keep exit 0 with warnings only for eligible degraded results; it does not permit unrecoverably incomplete output. For `translate-doc`, rejected documents preserve existing output, directory translation continues after per-document failures, and the command exits non-zero if any document failed.
- Source segments are bounded by the final request/context budget and by `[translation].max_source_tokens_per_segment` (default `384`, `0` disables). This conservative 7B-safe default prevents repeated retries of a segment that a deployed model ends early; raise it only after validating complete output. Profiled local-tokenizer plans include chat framing, prompt/context, assistant marker, completeness-retry reservation, and output reservation. Oversized fenced code/table blocks fail closed with `ProtectedBlockTooLarge` before a request is sent.
...
- Rendered translation prompts are versioned as `hy-mt2-prompts/v2`. This ID is recorded in inference fingerprints, task history, `--plan` token-budget diagnostics, and any benchmark metadata; increment it whenever model-visible prompt text changes so old cache entries and benchmark runs remain distinguishable.
