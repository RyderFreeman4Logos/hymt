...
- `--stream` always uses the streaming path. With the default `[translation].first_chunk_priority = false`, cached and missing segments are emitted as ordered Token events. When `first_chunk_priority = true`, remaining chunks start after segment 0's first backend token; on a terminal, segment 0 tokens are shown optimistically for low TTFT, while pipe/file stdout buffers segment 0 until completeness passes and emits leading untranslated blocks first. `--no-stream` keeps the existing buffered stdout behavior.
